...
def make_keyboard_ascii(image_path: Path, cols: int = 58) -> str:
    ...
    try:
        ...
        rect = (max(1, int(w * 0.06)), max(1, int(h * 0.03)), int(w * 0.88), int(h * 0.92))
    ...
    # Tighter crop around face/head-and-shoulders to make portrait feel larger
    cw, ch = crop.size
    face_top = int(ch * 0.02)
    face_bottom = int(ch * 0.78)
    crop = crop.crop((0, face_top, cw, face_bottom))
    mask_crop = mask_crop.crop((0, face_top, cw, face_bottom))

    rows = max(24, int(cols * crop.height / crop.width * 0.48))
    ...
    # Coarser keyboard-only character ramp
    chars = " .:-=+*#%@"
...
def main() -> None:
    ...
    parser.add_argument("--cols", type=int, default=58, help="ASCII width in characters")
